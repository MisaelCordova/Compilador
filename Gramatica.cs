...
{
    internal class Gramatica : IGramatica
    {
        ...
        {
            int[] linha = new int[producoes.GetLength(1)];
            for (int i = 0; i < linha.Length; i++)
            {
                linha[i] = producoes[numero - 1, i];
            }
            return linha;
        }
        public int parse(int producao, int token)
        {
            Console.WriteLine(producao + " " + token);
            return tabParsing[producao, token];
        }
        public int getFinalPilha()
        {
            return 52;
        }
        public int getVazio()
